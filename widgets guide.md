...
if a then --if a is not nil, or not false will return true
    print(a)
end
```
Loops and Tables
...
## Widget Flow
Before we start actually coding, we should just touch on how widgets tend to work. The process diagram summarises what a widget will do when running.
...
2) `Get mouse position on clicks` -----> `If rectangle has been drawn, calculate rectangle coords, find units that fall with in` -----> `Select all combat units (but not constructors and buildings) `
...
4) `Get position data of an enemy unit in LOS` -----> `predict its position in 3 seconds based on current movement` -----> `aim nearby friendly artillery to that spot. `

The last idea in this list (which too my knowledge I just made up), would likely give the user a big and unfair advantage, therefore be against the code of conduct, and not allowed. Better to check if unsure, before spending time making it.
...
1) `Read data from another files`. I.e. Find out info about something already in the game.
2) `Use a call-in provided by the widget wrapper` I.e. find out about an event that has just occurred in the game.

### Read Data from other Files

The engine keeps some lovely and accessible information about all sorts of stuff. There are many many many existing functions in spring engine that clever people have added in the past that give us a way of finding out about things in the game. In particular units and their stats, but also mouse coords, camera angles, team members, other widgets. A few important ones are listed below:

`UnitDefs[unitDefID]` -`UnitDefs` is a big kind of table that contains everything you will need to know about a unit type. From its name, cost, weapons, initial hp, animations, types, to custom parameters (where everything else goes). It is created outside the widget and is very useful. We will be using this later.
To access it, we need a `unitDefID` (not to be confused with `unitID`). Every unit in the game has a unique `unitID`. Find this, translate it to its `unitDefID`, and put it this the 'UnitDefs', and assuming you do it in the right way you can access these variables. 

`Get.Selection()` - returns the unitID of everything selected in game.
`Spring.TraceScreenRay()` Translates mouse screen position to map coordinates.

xxx add more

### Events in Game

Events can be many things, pretty much when anything interesting happens in game, an event is made. Generally, we will be interested in triggering parts of our code when a particular event occurs. Perhaps when a unit is created, destroyed or transferred to an ally, or perhaps when something is clicked, said, or even just every frame. They often bring along their own variables. A few are below, they can be found in XXX.lua. In widgets, we use them in the following format:
```lua
...
--Game Events:
function widget:UnitDestroyed(unitID, teamID) --widgets can only access this event for things destroyed in LOS. xxx more vars
end

function widget:UnitCreated(unitID, unitDefID, unitTeam, builderID) --whenever a new unit is created (includes resurections)
end
...
```
There are dozens of callins. The following are good places to see what there are, and what you can do with them (though they are not always up to date)

xxx

The following files also show widget options. (xxx widgets.lua etc)

We will now get onto coding... but before we can make a widget, we need software to help us...
...
1) You need Beyond all reason.
2) You need an IDE (Technically you could use notepad...). Most, including me, use VScode. XXX addon
3) You want a copy of BAR from Github. XXX details.

Know all the best resources! There is a list at the end of the guide, but you can save a lot of headaches and questions if you know the following sites.
XXX Springrts.com
XXX etc
XXX etc


# Starting on Our Widget

## Making a blank file and adding GetInfo()
Finally, we can start making widgets! Start by making a blank file in vscode, save it as `your_widget_name.lua`. Make sure there are no spaces.
...
This will be the start of all our widget files. As well as allowing the handler the rest of our code will go underneath. We will keep an order to how we code, both for usability and readability. We should also comment a lot as we go, as it's easy to forget what and why we do stuff.

## Planning the Widget
From this point on we will be coding an example. It's a good idea to check the feasibility of your idea, and that you have a rough idea of how your code will work before starting, as the options available to widgets are vast but not infinite.
...
Going back to the process flow list:
`A) Get some info from the engine` ----> `B) process this info within the widget` ----> `C) Some output or action`
...
We need the commander’s hit points. Fortunately for us, there's a method to get this:
...
`number health, number maxHealth, number paralyzeDamage, number captureProgress, number buildProgress` (if unit exist)
...
In lua, _ is used for stuff we don't care about, so effectively  we are only caring about the first two returned vars.

This will provide us what we need from the game for our widget, but for it to work we need to have the unitID of the commander.
There are many ways of getting this information, but we should consider a few things first before choosing ours. The answers will determine what code we need to use.
...
Putting this all together, and adding in the looping code we may end up with something like this: (note we are yet not using the code in this example)
...
Wehn fixed, and if all has gone to plan, this code will loop through all units we own, check if they are a commander, and if so grab their health. We record these values in a table called `myCommanderTable`, that we can access in a future function (as this was declared outside the function it is availble anywhere. within the widget) We are using a dictionary type table for `myCommanderTable`, that is the index is the `unitID`. As the `unitID` is unique to the unit, this will work for us.
...
Our biggest debugging tool is a highly helpful little chap called `Spring.Echo()`. Much like `print()` prints the output of code, `Spring.Echo()` will display in game whatever is within the ().
This is also recorded in the `infolog.txt`, along many other things each game. A finished widget should have all `Spring.Echo()` removed so as to not fill the info log up with anything.
...
For our checks, we want to look at our lists. I snuck in the echos to the end of our function, and we will use a callin called `MousePress()` to activate it on a mouse press.
`MousePress()` gives us mx and my coords of the mouse (we dont care about these today), and which `button` it pressed (1 = left click)
```lua
...
Mine looks like this, yours may differ. Turns out there are a fair number of units that identify as coms (the numbers in the first table).
The second table shows our starting com, unitID is 12052, current HP is 3700 and max is 3700. Cool. If this doesn't work then check the widget is enabled, and you have left clicked once after the game starts. Although infolog updates in real time, sometimes it only updates if there's several things to write to it. In a skirmish you can pause/unpause a few times if the `Spring.Echo()` isn't showing at the bottom yet (in a multiplayer game I minimise and maximise the screen which also writes to the log). If it's still not working ask for help in discord!
...
We can now move onto step 2), where we need to take the hit points and do any manipulations or checks, then get ready to display them.

BAR uses openGL to do it's drawing. If you don't know anything about openGL (like me), it's a pretty large topic, with many great guides.
Fortunately the devs have made many tools to help us here so we don't need to start from scratch, nor massively concern ourselves with how it works. We will make use of another widget that is shipped with the game, allowing use to use `font():`

Draw callins - that is those which let us draw stuff, are `DrawScreen()` for drawing on the screen (think the UI), or the world `DrawWorld()` for drawing to the map (think radar overlay). These are resource expensive, and we want to keep as much code away from them as possible, and only use them to send our graphics to be drawn. If we can reduce changing the drawing as much as possible, we greatly reduce the load on the CPU.
...
To make this work, we are going to create gl.list of exactly what we want to send to be drawn on screen. We will only update the list if something has changed (e.g. commander hit points changed), but we use whatever code we need wish whilst making the list without too much concern for CPU cost.

**Big Tip:** Its a very good idea to look over other widgets and see how they handle drawing. For simple text, font: will work for now.

Let's make a function to handle the list creation, and formatting. We will only call this function when something with the commander health has changed.
```lua
local healthToDraw
...
Notice first that we are creating a variable called healthToDraw outside of the function, this will become gl list. Inside the function, we declare it as `gl.CreateList(function()` which has an open bracket, and contains a `function()`. This bracket is closed after the first end, which represents the end of the function. We will put our code between these.
...
### Using font():
Writing characters in openGL directly is expensive. It's better to turn them into a texture and display that instead. Another widget deals with this for us without us needing to think about it. We need to reference the font widget when our widget loads. This is done in function `widget:Initialize()` - code will run once when the game is loaded.
We are loading the chosen font from a list of BAR fonts shipped with the game, and then using a previously created widget called 'fonts' which sets everything up. We don't need to know how or why this works right now!
```lua
...
```lua
local posX,posZ = 512,512 --should be visible somewhere on screen (unless you have a very strange resolution - you will deal with it later.)
local fontsize = 20
